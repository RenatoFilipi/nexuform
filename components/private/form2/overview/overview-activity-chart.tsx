...
const OverviewActivityChart = () => {
  ...
  const dateRange = useMemo(() => {
    const daysCount = getDateDifferenceInDays(pf.from, pf.to);
    return Array.from({ length: daysCount }, (_, i) => {
      const date = addDays(pf.from, i);
      return {
  ...
  useEffect(() => {
    if (!pf.submissionLogs || !pf.viewLogs) return;
    ...
    pf.submissionLogs.forEach((log) => {
      if (!log?.created_at) return;
      const logDate = parseISO(log.created_at);
      if (isWithinInterval(logDate, { start: pf.from, end: pf.to })) {
  ...
  }, [pf.submissionLogs, pf.viewLogs, dateRange, pf.from, pf.to]);
