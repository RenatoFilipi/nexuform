...
const SubmissionsList = () => {
  ...
  const [filteredSubmissions, setFilteredSubmissions] = useState(global.submissions);
  ...
  useQuery({
    ...
    queryFn: () => {
      if (status === "all") {
        setFilteredSubmissions(global.submissions);
        return null;
      }
      const f = global.submissions.filter((x) => x.status === status);
