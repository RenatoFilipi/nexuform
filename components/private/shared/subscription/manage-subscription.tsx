...
import { ArrowRight, Check, CheckIcon, CreditCard, Rocket, X } from "lucide-react";
        ...
        <DialogOverlay className="backdrop-blur-sm">
          <DialogContent className="flex flex-col w-full max-w-2xl max-h-[90%] h-full overflow-y-auto">
            <DialogHeader className="">
              <DialogTitle className="">{t("label_manage_sub")}</DialogTitle>
    ...
    <Drawer open={open} onOpenChange={setOpen}>
      ...
      <DrawerContent className="flex flex-col p-4 max-h-[90%]">
        <DrawerHeader className="text-left">
          <DrawerTitle className="">{t("label_manage_sub")}</DrawerTitle>
...
const PlanOption = ({ plan, isCurrent, onSelect }: { plan: IPlan; isCurrent: boolean; onSelect: () => void }) => {
  ...
  return (
    <div className={`relative p-4 rounded-lg border hover:border-primary hover:bg-primary/5`}>
