...
interface FormGroupProps2 {
  mode: mode;
  form: FormModel;
}

const FormGroup2 = ({ mode, form }: FormGroupProps2) => {
  const { blocks, primary_color, submit_label, name, description } = form;
  const currentColor =
    design.find((x) => x.label === primary_color) ?? design[0];
